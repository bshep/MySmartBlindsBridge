...
#define HAPOSITION_CALLBACK(name) void (*name)(HANumeric cmd, HACover* sender)
...
public:
     ...
     * @param callback
     */
    inline void onPosition(HAPOSITION_CALLBACK(callback))
...
private:
    ...
    void handlePosition(const uint8_t* cmd, const uint16_t length);
    ...
    HAPOSITION_CALLBACK(_positionCallback);
};
