...
void HACover::buildSerializer()
{
    ...
    if (_features & PositionFeature) {
        ...
        // _serializer->set(AHATOFSTR("position_template"),"{{ value_json.position }}");
        _serializer->topic(AHATOFSTR("set_position_topic"));
    }
}
...
void HACover::onMqttConnected()
{
    ...
    subscribeTopic(uniqueId(), AHATOFSTR("set_position_topic"));
}
...
)
{
    ...
    if (HASerializer::compareDataTopics(
        topic,
        uniqueId(),
        AHATOFSTR("set_position_topic")
    )){
        handlePosition(payload, length);
    }
}
...
void HACover::handlePosition(const uint8_t* cmd, const uint16_t length)
{
    if (!_positionCallback) {
        return;
    }

    HANumeric position = HANumeric().fromStr(cmd, length);

    _positionCallback(position, this);
}
